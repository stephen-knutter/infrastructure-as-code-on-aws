...
if (input['user-add'] === true) {
	...
	if (input['--overdue'] === true) {
		params.FilterExpression = "due < :yyyyddmm";
		params.ExpressionAttributeValues[':yyyyddmm'] = {"N": moment().format("YYYYMMDD")};
		...
		params.FilterExpression = "due > :yyyyddmm";
		params.ExpressionAttributeValues[':yyyyddmm'] = {"N": input['--dueafter']};
	} else if (input['--duebefore'] !== null) {
		params.FilterExpression = "due < :yyyyddmm";
		params.ExpressionAttributeValues[':yyyyddmm'] = {"N": input['--duebefore']};
	}
	if (input['<category>'] !== null) {
